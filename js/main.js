/**
 * Main application script for Clinical Trials website
 */
class ClinicalTrialsApp {
  ...
  async init() {
    ...
    try {
      ...
      this.searchFilter.populateStudyTypeFilter();
    ...
    const cardHTML = `
      <div class="trial-card" data-trial-id="${trial.id}" onclick="window.clinicalTrialsApp.viewTrialDetail('${trial.id}')">
        ...
        <div class="trial-card-footer">
          ...
          <a href="trial-detail.html?id=${trial.id}" class="trial-view-btn" onclick="event.stopPropagation();">
            View Details
          </a>
        </div>
      </div>
  ...
  viewTrialDetail(trialId) {
    window.location.href = `trial-detail.html?id=${trialId}`;
  }
