class PhysicianLoginPage {
  constructor() {
    this.redirectUrl = 'physician-community-php.html';
  }
  ...
  async redirectIfAuthenticated() {
    try {
      ...
      console.error('Failed checking physician session:', error);
    }
  }

  bindEvents() {
    document.getElementById('physicianLoginForm')?.addEventListener('submit', event => {
  ...
  async handleLogin() {
    ...
    try {
      const response = await fetch('api/auth.php?action=physician_login', {
        method: 'POST',
        credentials: 'same-origin',
        headers: {
      ...
      console.error('Physician login failed:', error);
...
document.addEventListener('DOMContentLoaded', async () => {
  const page = new PhysicianLoginPage();
  await page.init();
});
